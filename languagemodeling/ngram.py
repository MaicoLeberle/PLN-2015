...
class NGram(EvaluatingClass):
    ...
    def cond_prob(self, token, prev_tokens=None):
        ...
        ngram = tuple((['<s>'] *
                      (self.n - 1 - len(prev_tokens))) + prev_tokens + [token])
...
class InterpolatedNGram (EvaluatingClass):
    ...
    def cond_prob(self, token, prev_tokens=None):
        ...
        for i in range(1, self.n + 1):
            prev_lambda = 1 - sum(acc_lambdas)
            current_lambda = self.__get_lambda(tokens=prev_tokens[(i-1):],
                                               prev_lambdas=prev_lambda)
            try:
                aux = self.__max_likelihood_prob(token,
                                                 prev_tokens[(i-1):])
            except ZeroDivisionError:
                # Do not add this term.
                pass
            else:
                res += current_lambda * aux
    ...
    def __get_lambda(self, tokens, prev_lambdas=1):
        ...
        if (len(tokens) == self.n - 1):
            ...
            return (self.count(tokens) /
                    float(self.count(tokens) + self.gamma))
        elif (len(tokens) == 0):
            # Last lambda coefficient
            return (prev_lambdas)
        else:
            return (prev_lambdas * self.count(tokens) /
                    float(self.count(tokens) + self.gamma))
    ...
    def __compute_log_likelihood(self):
        ...
        for key, value in self.counts_prime.items():
            log_likelihood += value * log(self.cond_prob(key[-1],
                                          key[:-1]))
...
class BackOffNGram(EvaluatingClass):
    ...
    def cond_prob(self, token, prev_tokens=None):
        ...
        if (prev_tokens == []):
            ...
            if (token in self.A(prev_tokens)):
                ...
                try:
                    res = self.__max_likelihood_prob(token,
                                                     prev_tokens[1:])
            ...
            if (token in self.A(prev_tokens)):
                aux1 = self.__count_star(list(prev_tokens) + [token])
                aux2 = float(self.count(prev_tokens))
                res = aux1 / aux2
            else:
                try:
    ...
    def denom(self, tokens):
        ...
        if (len(tokens) > 1):
            for token in self.A(tokens):
                sum_cond_prob += self.cond_prob(token, tokens[1:])
        else:
            for token in self.A(tokens):
        ...
        for key, value in self.counts_prime.items():
            ...
            try:
                log_likelihood += value * \
                    log(self.cond_prob(key[-1], key[:-1]))
            except ValueError:
    ...
    def __max_likelihood_prob(self, token, prev_tokens=None):
        ...
        if (prev_tokens == []):
            return self.unigram_model.cond_prob(token, prev_tokens)
        else:
            ngram = tuple((['<s>'] * (self.n - 1 - len(prev_tokens)))
                          + list(prev_tokens) + [token])
            return (self.count(ngram) / float(self.count(tuple(prev_tokens))))
