...
class InterpolatedNGram:
    ...
    def __init__(self, n, sents, gamma=None, addone=True):
        ...
        assert n > 0
        self.n = n
        # self.addone will be needed when computing self.cond_prob
        self.addone = addone
        if gamma is None:
            # If gamma is not provided, use 90% of sents as training set, and
            # last 10% as held-out data to estimate gamma.
            training = sents[:int(len(sents) * 0.9)]
            held_out = sents[int(len(sents) * 0.9):]
            
            # Compute gamma from held-out data.
            # self.gamma = ???
        else:
    ...
    def __max_likelyhood_prob(self, token, prev_tokens=None):
        ...
        if self.addone and prev_tokens == []:
            # Implementing add-one smoothing for the lower level (unigrams)
            return ((self.count(ngram) + 1) / float(self.count(tuple()) + \
                                                            self.__V()))
        else:
            if self.count(ngram) == 0:
                # Result will be 0, even if self.count(tuple()) == 0
                # (in which case this call to cond_prob will be undefined).
                return (0)
            else:
                return (self.count(ngram) / float(self.count(tuple(prev_tokens))))
    ...
    def cond_prob(self, token, prev_tokens=None):
        ...
        for i in range(self.n):
            current_lambda = self.__get_lambda(tokens=prev_tokens, i=i, gamma=self.gamma, prev_lambdas=prev_lambdas)
            prev_lambdas = prev_lambdas - current_lambda
            res += current_lambda * self.__max_likelyhood_prob(token=token, prev_tokens=prev_tokens[:len(prev_tokens) - i])
